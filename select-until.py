...
def find_matching_point(view, sel, selector):
    ...
    if result is None:
        return safe_end(view.find(selector, sel.end() + 1, sublime.LITERAL))
    ...
    if not isReverse:
        if num is not None:
            ...
            return safe_end(view.find(regex, sel.end() + 1))
        else:
            return safe_end(view.find(chars, sel.end(), sublime.LITERAL))

    else:
        if num is not None:
            return sel.begin() - num
        elif regex is not None:
...
def negate_selector(selector):
    ...
    if num is not None:
        return makeReverse, negateChar + "{" + num + "}"
    elif chars is not None:
        return makeReverse, negateChar + "[" + chars + "]"
    elif regex is not None:
        return makeReverse, negateChar + "/" + regex + "/"
...
def on_change(view, oriSels, selector, extend):
    ...
    for sel in oriSels:
        ...
        if point is -1:
            point = sel.b  # try to keep this selection the same
...
class SelectUntilArgCommand(sublime_plugin.TextCommand):
    def run(self, edit, extend, selector):
        oriSels = [sel for sel in self.view.sel()]
        on_change(self.view, oriSels, selector, extend)
        on_done(self.view, extend)
...
class SelectUntilCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, extend):
        ...
        v = view.window().show_input_panel(
            ...
            SelectUntilCommand.prevSelector, lambda selector: on_done(view, extend),
            lambda selector: on_change(view, oriSels, selector, extend), lambda: on_cancel(view, oriSels)
        )
